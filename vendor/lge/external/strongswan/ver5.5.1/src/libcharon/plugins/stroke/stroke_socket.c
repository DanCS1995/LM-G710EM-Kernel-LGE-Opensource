...
static void stroke_add_conn(private_stroke_socket_t *this, stroke_msg_t *msg)
{
	...
	DBG1(DBG_CFG, "  eap_identity=%s", msg->add_conn.eap_identity);
	DBG1(DBG_CFG, "  aaa_identity=%s", msg->add_conn.aaa_identity);
	DBG1(DBG_CFG, "  xauth_identity=%s", msg->add_conn.xauth_identity);
	DBG1(DBG_CFG, "  ike=%s", msg->add_conn.algorithms.ike);
	DBG1(DBG_CFG, "  esp=%s", msg->add_conn.algorithms.esp);
	DBG1(DBG_CFG, "  ah=%s", msg->add_conn.algorithms.ah);
	DBG1(DBG_CFG, "  dpddelay=%d", msg->add_conn.dpd.delay);
	DBG1(DBG_CFG, "  dpdtimeout=%d", msg->add_conn.dpd.timeout);
	DBG1(DBG_CFG, "  dpdaction=%d", msg->add_conn.dpd.action);
	DBG1(DBG_CFG, "  closeaction=%d", msg->add_conn.close_action);
	DBG1(DBG_CFG, "  mediation=%s", msg->add_conn.ikeme.mediation ? "yes" : "no");
	DBG1(DBG_CFG, "  mediated_by=%s", msg->add_conn.ikeme.mediated_by);
	DBG1(DBG_CFG, "  me_peerid=%s", msg->add_conn.ikeme.peerid);
	DBG1(DBG_CFG, "  keyexchange=ikev%u", msg->add_conn.version);
