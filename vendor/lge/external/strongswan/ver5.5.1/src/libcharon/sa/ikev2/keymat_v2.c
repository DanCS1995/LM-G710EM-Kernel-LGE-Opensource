							...
							uint16_t key_size, prf_plus_t *prf_plus)
{
   ...
   DBG_WITH_ANDROID_PROPERTY("persist.net.wo.key.display", DBG_KEY, "Sk_ei secret %B", &sk_ei);
   this->sk_ei = chunk_create_clone(malloc(sk_ei.len), sk_ei);
   ...
   DBG_WITH_ANDROID_PROPERTY("persist.net.wo.key.display", DBG_KEY, "Sk_er secret %B", &sk_er);
   this->sk_er = chunk_create_clone(malloc(sk_er.len), sk_er);
					...
					uint16_t enc_size, uint16_t int_alg, prf_plus_t *prf_plus)
{
   ...
   //	DBG4(DBG_IKE, "Sk_ai secret %B", &sk_ai);
    DBG_WITH_ANDROID_PROPERTY("persist.net.wo.key.display", DBG_KEY, "Sk_ai secret %B", &sk_ai);
   this->sk_ai = chunk_create_clone(malloc(sk_ai.len), sk_ai);
   ...
   DBG_WITH_ANDROID_PROPERTY("persist.net.wo.key.display", DBG_KEY, "Sk_ar secret %B", &sk_ar);
   this->sk_ar = chunk_create_clone(malloc(sk_ar.len), sk_ar);
	...
	pseudo_random_function_t rekey_function, chunk_t rekey_skd)
{
	...
	uint16_t alg, key_size, int_alg, enc_key_size;
   ...
   DBG_WITH_ANDROID_PROPERTY( "persist.net.wo.key.display", DBG_KEY, "SPI_i %B", &spi_i);
   DBG_WITH_ANDROID_PROPERTY( "persist.net.wo.key.display", DBG_KEY, "SPI_r %B", &spi_r);
    ...
    DBG_WITH_ANDROID_PROPERTY("persist.net.wo.key.display", DBG_KEY, "SKEYSEED %B", &skeyseed);
