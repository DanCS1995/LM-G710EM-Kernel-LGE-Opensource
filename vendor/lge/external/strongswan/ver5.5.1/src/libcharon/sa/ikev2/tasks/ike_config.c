...
static configuration_attribute_t *build_imei(const char *imei)
{
	configuration_attribute_type_t type = get_cust_setting_int(CUST_IMEI_CP);
	chunk_t chunk;
	size_t i;
	u_char *bin = malloc(9);
							  ...
							  configuration_attribute_t *ca)
{
	...
	switch (ca->get_type(ca))
	{
		...
		default:
		{
			...
			else if (ca->get_type(ca) == get_cust_setting_int(CUST_IMEI_CP)) {
	...
	private_ike_config_t *this, message_t *message)
{
	...
	{	/* in first IKE_AUTH only */
		...
		if (config->get_imei(config)) {
			...
			cp->add_attribute(cp, build_imei(config->get_imei(config)));
		}
		//	add netmask
