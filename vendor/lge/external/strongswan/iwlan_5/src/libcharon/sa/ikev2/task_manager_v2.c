	...
	private_task_manager_t *this, u_int32_t message_id)
{
		...
		array_count(this->initiating.packets))
	{
		...
		if (mobike == NULL)
		{
			...
			if (pcas_is_operator("ORG",PCAS_SLOT_PRIMARY) && this->initiating.retransmitted < 3 && is_dpd) {
...
#ifdef __ANDROID__
		if (errortype != 0) {
			int len = strlen(this->ike_sa->get_name(this->ike_sa) + 4);
			if (len >= sizeof(apn)) {
				DBG1(DBG_ENC, "Failed to copy apn, sizeof(apn)=%d, len(get_name)=%d", sizeof(apn), len);
				len = sizeof(apn) -1;
			}
			strncpy(apn, (this->ike_sa->get_name(this->ike_sa) + 4), len);
			if (errortype == NETWORK_FAILURE) {
				char key2[PROP_NAME_MAX];
				char value2[PROP_VALUE_MAX];
				snprintf(key2, PROP_NAME_MAX, "net.wo.epdg.bo_timer_%s", apn);
				...
				(errortype == INVALID_KE_PAYLOAD))
			{
				...
				char operator[PROP_VALUE_MAX] = "";
				property_get("ro.build.target_operator", operator, "NONE");
				if (pcas_is_operator("ORG",PCAS_SLOT_PRIMARY) && this->retransmit_tries == 0)
				{
...
task_manager_v2_t *task_manager_v2_create(ike_sa_t *ike_sa)
{
	...
	if (get_cust_setting(IKE_RETRAN_TRIES, sysvalue))
	{
	...
	if (get_cust_setting(IKE_RETRAN_TO, sysvalue))
	{
	...
	if (get_cust_setting(IKE_RETRAN_BASE, sysvalue))
	{
