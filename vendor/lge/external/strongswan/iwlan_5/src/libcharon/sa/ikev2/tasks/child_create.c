								   ...
								   bool no_dh, bool ike_auth)
{
	...
	{
		if (pcas_is_operator("ORG", PCAS_SLOT_PRIMARY))
		{
...
#ifdef ANDROID
				if (get_cust_setting_bool(FORCE_TSI_64))
				{
	...
	else
	{
		...
		if (get_cust_setting_bool(FORCE_TSR_FULL)) {
			enumerator_t *enumerator = this->ike_sa->get_virtual_ips(this->ike_sa);
			host_t *vip;
			while (enumerator->enumerate(enumerator, &vip)) {
				int i = 0;
	...
	private_child_create_t *this, message_t *message)
{
	...
	{	/* no virtual IPs configured */
		...
		if (get_cust_setting_bool(REKEY_FULL))
		{
			if (this->reqid)
			{
				u_int8_t has_v4 = 0;
